...
from models.resnet import (
    SupConResNet,
    LinearClassifier,
    LinearClassifier_MultiLabel,
    SupConResNet_Original,
    SupConResNet_Original_Headless,
)
...
from dataset.biomarker_multi_complete import (
    BiomarkerDatasetAttributes_MultiLabel_Complete,
)
from dataset.biomarker_multi_fusion import (
    BiomarkerDatasetAttributes_MultiLabel_MultiClass,
)
...
def set_loader_new(opt):
    ...
    if opt.dataset == "OCT":
        ...
        test_dataset = OCTDataset(
            csv_path_test, data_path_test, transforms=train_transform
        )
        ...
        if opt.super == 2 and opt.multi == 0:
            train_dataset = BiomarkerDatasetAttributes_Fusion(
                csv_path_train, data_path_train, transforms=train_transform
            )
            test_dataset = BiomarkerDatasetAttributes_Fusion(
                csv_path_test, data_path_test, transforms=val_transform
            )
        elif opt.super == 2 and opt.multi == 1:
            train_dataset = BiomarkerDatasetAttributes_MultiLabel_MultiClass(
                csv_path_train, data_path_train, transforms=train_transform
            )
            test_dataset = BiomarkerDatasetAttributes_MultiLabel_MultiClass(
                csv_path_test, data_path_test, transforms=val_transform
            )
        elif opt.multi == 1 and (opt.super == 5 or opt.super == 8):
            train_dataset = BiomarkerDatasetAttributes_MultiLabel_Complete(
                csv_path_train, data_path_train, transforms=train_transform
            )
            test_dataset = BiomarkerDatasetAttributes_MultiLabel_Complete(
                csv_path_test, data_path_test, transforms=val_transform
            )
        elif opt.multi == 1 and opt.super != 3:
            ...
            test_dataset = BiomarkerDatasetAttributes_MultiLabel(
                csv_path_test, data_path_test, transforms=val_transform
            )

        else:
            train_dataset = BiomarkerDatasetAttributes(
                csv_path_train, data_path_train, transforms=train_transform
            )
            test_dataset = BiomarkerDatasetAttributes(
                csv_path_test, data_path_test, transforms=val_transform
            )
    else:
