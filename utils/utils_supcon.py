...
def set_loader(opt):
    ...
    if opt.dataset == "OCT":
        ...
        opt.dataset == "Prime"
        or opt.dataset == "CombinedBio"
        or opt.dataset == "CombinedBio_Modfied"
        or opt.dataset == "Prime_Compressed"
    ):
        mean = 0.1706
        std = 0.2112
    elif (
        opt.dataset == "TREX_DME"
        or opt.dataset == "Prime_TREX_DME_Fixed"
        or opt.dataset == "Prime_TREX_Alpha"
    ...
    if opt.dataset == "OCT":
        ...
        csv_path_train = (
            ...
            + "/complete_prime_recovery_trex"
            + "/full_prime_train.csv"
        )
        data_path_train = opt.train_image_path
        train_dataset = PrimeDatasetAttributes(
            csv_path_train,
            data_path_train,
            transforms=TwoCropTransform(train_transform),
        )
        ...
        opt.dataset == "Prime_TREX_DME_Fixed"
        or opt.dataset == "Prime_TREX_Alpha"
        or opt.dataset == "Patient_Split_2_Prime_TREX"
        or opt.dataset == "Patient_Split_3_Prime_TREX"
    ):
