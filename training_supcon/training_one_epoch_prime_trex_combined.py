    ...
    for idx, (images, bcva, cst, eye_id, patient) in enumerate(train_loader):
        ...
        if opt.num_methods == 0:
            ...
            if opt.method2 == "SuperClass":
                criterion = SupConLoss_SuperClassDistance(temperature=0.07)
                loss = criterion(features, super_labels=labels1)
            elif opt.method2 == "SuperClass_Combined":
                criterion2 = SupConLoss_SuperClassDistance(temperature=0.07)
            ...
            loss = (
                criterion(features, labels1)
                + criterion(features, labels2)
                + criterion(features, labels3)
            )
